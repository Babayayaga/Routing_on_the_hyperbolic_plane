...
The `RoutingScenarioGraphicsItem` keeps track of the current positions of each `Vertex_handle`. The map `t_map` assigns to each `Vertex_handle` a `Point_2`, that represents its current position. 
