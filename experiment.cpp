...
void benchmark_tea() {
    ...
    for(int i = 0; i < trials; ++i) {
        CGAL::Timer timer;
        timer.start();
        const double average_time_orientation_test = routing_scenario.build_visibility_graph();
        timer.stop();

        if(i == 0) {
            std::cout << "average time orientation test: " << average_time_orientation_test << std::endl;
        }
